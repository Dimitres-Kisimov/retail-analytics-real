"""Honest EDA figures, saved to figures/.
...
All figures are light-mode PNGs using one validated palette (single-hue blue for
magnitude, categorical slots only where series identity matters).
"""
...
# Validated light-mode palette (see docs/BUSINESS_CASE.md for provenance).
BLUE = "#2a78d6"
GREEN = "#008300"
MAGENTA = "#e87ba4"
YELLOW = "#eda100"
...
def _new_axes(figsize: tuple[float, float] = (9, 4.5)) -> tuple[plt.Figure, plt.Axes]:
    _style()
...
def _save(fig: plt.Figure, out: Path, tight: bool = True) -> Path:
...
def fig_monthly_revenue(sales: pd.DataFrame, out_dir: Path = FIGURES) -> Path:
    ...
    return _save(fig, out_dir / "monthly_revenue.png")
...
def fig_top_products(sales: pd.DataFrame, out_dir: Path = FIGURES, n: int = 10) -> Path:
    ...
    fig, ax = _new_axes(figsize=(9, 4.8))
    ...
    if any("PAPER CRAFT" in str(desc) for _, desc in top.index):
        ...
        return _save(fig, out_dir / "top_products.png", tight=False)
    return _save(fig, out_dir / "top_products.png")
...
def fig_top_countries(sales: pd.DataFrame, out_dir: Path = FIGURES, n: int = 10) -> Path:
    ...
    return _save(fig, out_dir / "top_countries.png")
...
def fig_order_values(sales: pd.DataFrame, out_dir: Path = FIGURES) -> Path:
    ...
    return _save(fig, out_dir / "order_values.png")
...
def fig_returns_rate(sales: pd.DataFrame, returns: pd.DataFrame, out_dir: Path = FIGURES) -> Path:
    ...
    return _save(fig, out_dir / "returns_rate.png")
...
def fig_missing_customers(sales: pd.DataFrame, out_dir: Path = FIGURES) -> Path:
    ...
    ax.plot(x, share.to_numpy(), color=YELLOW, linewidth=2, marker="o", markersize=4)
    ax.set_xticks(x)
    ...
    return _save(fig, out_dir / "missing_customer_share.png")
